...
toDoList.prototype = {
  ...
  loadTodos: function() {
    ...
    if (!previousToDos) {
      $('main').prepend($(templates['header']({listTitle: 'All Todos', total: 0})));
      return;
    } else {
      $('main table').empty();
      var self = this;
  ...
  loadMenus: function() {
    ...
    var completeByDate = this.todosByDate(completeTodos);
    var currentSelected;

    var allTodosContext = { 
      total_all: self.collection.length, 
      all_todo: this.todosByDate(self.collection)
    }
  ...
  insertTodo: function(todo) {
    ...
    var content = templates['single_todo']({id: todo.id, title: todoDescription});

    if (todo.complete)  {
      $('#complete_todos').append(content);
      $('#complete_todos').find('[data-id="' + String(todo.id) +'"]').addClass('completed');
    } else {
      $('#incomplete_todos').append(content);
    }

    $('h1 div.circle').text(this.collection.length);
  },
  ...
  setMonthYear: function(month, year) {
    ...
    if (month === null || year === null) {
      ...
      month = ('0' + String(months.indexOf(month))).slice(-2);
      year = year.slice(2);
      date = month + "/" + year;
    }
  ...
  delete: function(e) {
    ...
    $todo.remove();
    $('table div.circle, main div.circle').text(this.collection.length);
    this.loadMenus();
  },
  ...
  getToDo: function(id) {
    var todo = this.collection.filter(function(item) {
  ...
  updateMainList: function(todos, title) {
    ...
    $('main').prepend($(templates['header']({listTitle: title, total: todos.length})));
  },
  ...
  changeList: function(e) {
    ...
    if (dateID === 'Completed') {
      ...
      todos = this.collection.filter(function(item) {
        if ($row.closest('table').is('#completed')) {
